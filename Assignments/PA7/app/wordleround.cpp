...
bool validHardModeGuess(const QString& guess, const bool correctLetters[]) {
    for (int i = 0; i < NUM_LETTERS; ++i) {
        if (correctLetters[i]) {
            if (!guess.contains(static_cast<char>('a' + i))) {
                return false;
            }
        }
    }
...
void WordleRound::registerEnterKey() {
    ...
    for (int i=0; i<length; ++i) {
         ...
         */
        for (int i = 0; i < length; i++) {
            if(colors[i] == YELLOW || colors[i] == GREEN){
                correctLetters[guess[i].toLower().toLatin1() - 'a'] = 1;
            }
            else{
                correctLetters[guess[i].toLower().toLatin1() - 'a'] = 0;
            }
        }
    }
